...
학생은 공통 사이트에서 **반·번호·4자리 PIN**을 입력합니다. 선생님 관리실의 **접속 정보**에서 학생별 PIN을 확인하거나 재발급합니다. 안내 카드에도 PIN이 인쇄됩니다. PIN을 5회 틀리면 선생님이 재발급할 때까지 잠깁니다. 기존 링크도 사용할 수 있습니다. 이미 4반 설치를 완료했다면 `202609070003_student_pin.sql`을 한 번 적용하고 Edge Function과 화면을 업데이트합니다.
...
마이그레이션 폴더의 첫 번째·두 번째·세 번째 SQL이 순서대로 적용됩니다. CLI로 처음 설치한 경우 이후 `db push`를 반복해도 기존 기록을 초기화하지 않습니다. 같은 SQL 전체를 SQL Editor에서 다시 실행하지 마세요.
...
4. 공통 사이트 주소와 학생별 PIN을 각 가정에 전달합니다. PIN은 해당 학생에게만 알려주세요.
5. 학생은 반·번호·PIN으로 접속해 사진·달 모양·관찰 시각·메모를 저장합니다. 전용 링크·QR도 계속 사용할 수 있습니다.
...
학생 PIN도 백업에 포함되지 않습니다. 전체 복원 시 새 PIN이 생성되므로 관리실에서 확인해 다시 알려주세요.
